...
import {
  generateErrorToast,
  generateToast
} from '@components/page_components/CheckoutPageComponents/util/toast';
import { useUserState } from 'contexts/user-state';
import { useSSOState } from 'contexts/sso-state';
import { getHappinStreamRoom } from 'lib/api/livestream';
import { exchangeCrowdcoreToken, getFollowed, postFollow, removeFollowed } from 'lib/api/user';
...
const Playback = () => {
    ...
    <div className="live-stream__container">
      ...
      {isLoading ?
        ...
        <div className="live-stream__inner xl:rounded-lg">
          <div
            className="relative flex-1 flex flex-col min-w-0 md:overflow-x-hidden hide-scrollbar">
            <div className="live-stream__video-bar">
              ...
              <div className="flex items-center sm:bg-gray-800 px-4 h-16 footer-action">
                ...
                <div className="flex-1 mx-4">
                  <div
                    className="font-semibold text-gray-50">{eventData?.eventID.creator.name}</div>
                </div>
                {
                  <button
            ...
            <div className="relative flex-1 xl:aspect-w-16 xl:aspect-h-9">
              <div className="absolute inset-0 bg-black">
                {showMerch &&
                  <div onClick={handleMerchandise}
                       className="merchandise-btn absolute right-3 top-3 inline-flex justify-center items-center w-10 h-10 bg-black bg-opacity-30 rounded-full z-10 transition cursor-pointer hover:bg-opacity-40">
                    <svg width="24px" height="24px" viewBox="0 0 24 24">
                      <path fill="#ffc646" stroke="#ffc646" strokeWidth="2" strokeLinejoin="round"
                            strokeMiterlimit="2" d="M3,6.3v14.2
  c0,0.552,0.448,1,1,1h16c0.552,0,1-0.448,1-1V6.3H3z" />
                      <path fill="none" stroke="#ffc646" strokeWidth="2" strokeLinecap="round"
                            strokeLinejoin="round" strokeMiterlimit="2" d="
  M21,6.3l-2.833-3.8H5.833L3,6.3l0,0" />
                      <path fill="none" stroke="#121212" strokeWidth="2" strokeLinecap="round"
                            strokeLinejoin="round" strokeMiterlimit="2" d="
  M15.778,9.6c0,2.099-1.691,3.8-3.778,3.8s-3.778-1.701-3.778-3.8" />
                ...
                <div
                  className="relative flex items-center justify-center w-full h-full pb-24 sm:pb-0">
                  <video className="w-full h-full" width="100%" height="100%" playsInline
                         autoPlay={false} muted={false} id="loop_video" controlsList="nodownload"
                         controls>
          ...
          <div
            className="hidden sm:flex sm:h-auto flex-col w-full md:w-80 border-l border-black bg-gray-700">
            <div
              className="items-center justify-center h-12 text-white border-b border-gray-800 font-semibold hidden md:flex">Playback
              List
            </div>
            <div className="live-stream__chat-room">
              <div className="pt-3 pb-1.5">
                {recordings?.map((item: any, index: number) => {
                  return (
                    <div
                      className={`recording-item flex p-4 cursor-pointer ${recordingVideoUrl === item.videoURL && 'active'}`}
                      key={index} onClick={() => handleChangeRecording(item)}>
      ...
      >
        <div className="mt-1 mb-2 px-2">
          {eventData?.recordings?.map((item: any, index: number) => {
            return (
              <div className="recording-item flex p-4 cursor-pointer" key={index}
                   onClick={() => handleChangeRecording(item)}>
                <svg width="16px" viewBox="0 0 24 24">
                  <g>
                    <path fill="#fff"
                          d="M3.9,18.9V5.1c0-1.6,1.7-2.6,3-1.8l12,6.9c1.4,0.8,1.4,2.9,0,3.7l-12,6.9C5.6,21.5,3.9,20.5,3.9,18.9z" />
