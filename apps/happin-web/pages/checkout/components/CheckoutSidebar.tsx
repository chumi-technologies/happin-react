  ...
  const bundleList = [
    ...
    },
    {
      ...
      imgList: [
        ...
        {
          src: 'https://images.unsplash.com/photo-1628755840182-11c29f38b44c?ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&ixlib=rb-1.2.1&auto=format&fit=crop&w=1650&q=80',
          loading: 'lazy',
          alt: 'img-03'
        }
      ]
      ...
      <div className="flex flex-col flex-1 h-0">
        <div className="px-6">
          <div className="relative flex items-start py-6 border-b border-solid border-gray-700">
            ...
            <div className="absolute -right-2 top-4 flex items-center justify-center w-8 h-8 rounded-full hover:text-rose-500 transition cursor-pointer" onClick={onClose}>
            ...
            bundleList.map((item, index) => (
              <div className="checkout__goods-item" key={index}>
                ...
                <div className="flex-1 min-w-0 ml-5">
                  ...
                  {
                    item.type && (
                      <div className="inline-block py-1 px-3 border-2 border-solid border-gray-600 bg-gray-700 text-gray-400 text-sm rounded-lg cursor-default">{item.type}</div>
                    )
                  }
                  {
                    item.select && (
                      <Select data={item.select} defaultValue="Tom Cook" onChange={(data) => {
                        console.log(data);
                      }} />
                    )
                  }
