    ...
    <>
      ...
      <div className="common__body">
        <div className="flex flex-col h-full">
          <MyEventsHead loading={loading} upcomingTickets={upcomingTickets}
                        pastTickets={pastTickets} savedTickets={savedTickets} />
                ...
                {!loading ?
                  <VStack alignItems="stretch" spacing={{ base: 5, sm: 8 }}>
                    {upcomingTickets && upcomingTickets.length === 0
                      ...
                      && <div
                        className="mb-5 font-semibold text-xl sm:text-xl">{`You did not join event, you can explore more events on Happin app. OR you can "create your event"`}.</div>}
                    <div id="upcoming">
                      <div
                        className="mb-5 font-semibold text-xl sm:text-2xl">{upcomingTickets && upcomingTickets.length > 0 && `Upcoming`}</div>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 lg:grid-cols-3">
                        {
                          upcomingTickets && upcomingTickets.length > 0 && upcomingTickets.map(item =>
                            <MyEventItem data={item} key={item._id} />)
                        }
                      </div>
                    </div>
                    <div id="past">
                      <div
                        className="mb-5 font-semibold text-xl sm:text-2xl">{pastTickets && pastTickets.length > 0 && `Past`}</div>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 lg:grid-cols-3">
                        {
                          pastTickets && pastTickets.length > 0 && pastTickets.map(item =>
                            <MyEventItem data={item} key={item._id} />)
                        }
                      </div>
                    </div>
                    <div id="saved">
                      <div
                        className="mb-5 font-semibold text-xl sm:text-2xl">{savedTickets && savedTickets.length > 0 && `Saved`}</div>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 lg:grid-cols-3">
                        {
                          savedTickets && savedTickets.length > 0 && savedTickets.map(item =>
                            <MyEventItem data={item} key={item._id} />)
                        }
                      </div>
                    </div>
