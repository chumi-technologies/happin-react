...
import { get, crowdCoreApiHost, happinApiHost } from './base';
...
const getEventDetail = async (eventId: string, source: string) => {
    const response = await get<EventResponse>(EVENT_DETAIL_PATH.replace('{event_id}', eventId).replace('{source}', source), happinApiHost)
    return response || {}
}
