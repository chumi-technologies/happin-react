...
const SELL_BY_CARD_OR_BY_CASH_PATH = '/partnership/sell-by-card-or-cash?partnerId=${partnerId}&acid=${acid}'
...
const getSellByCardOrByCash = async(partnerId:string,acid:string)=> {
   const response = await getFromCrowdCore(SELL_BY_CARD_OR_BY_CASH_PATH.replace('${partnerId}',partnerId).replace('${acid}',acid))
   return response || {};
}
export { getConnectedTeam, swtichTeam, getSaasUserInfo, getSellByCardOrByCash }